...
func newQQGroup(name string, info interface{}) source.Source {
	...
	if err != nil {
		fmt.Println("QQGroup db connect err")
		return nil
	}
...
func (q *qqGroup) Search(key interface{}, resChan chan result.Result, wg *sync.WaitGroup) {
	num := key.(int)
	fmt.Println("Search QQGroup, key=", num)
	done := &sync.WaitGroup{}
	done.Add(3)
...
func (q *qqGroup) searchMemberByQQNum(qqNum int, resChan chan result.Result, done *sync.WaitGroup) {
...
func (q *qqGroup) searchMemberByGroupNum(groupNum int, resChan chan result.Result, done *sync.WaitGroup) {
	var memberRes []member
...
func (q *qqGroup) searchGroupByGroupNum(groupNum int, resChan chan result.Result, done *sync.WaitGroup) {
	var groupRes []group
